...
(function () {
    ...
    AdvancedAnalytics.prototype.generateImprovementPlan = function () {
        try {
            ...
            try {
                ...
                console.error('Error calculating current grade:', e.message);
            }

            var targetGrade = 'B+';
            try {
                targetGrade = this.getTargetGrade ? this.getTargetGrade(currentGrade) : 'B+';
            } catch (e) {
                console.error('Error getting target grade:', e.message);
            }
            ...
            var plan = {
                ...
                estimatedCompletion: this.getCompletionDate ? this.getCompletionDate(6) : new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            ...
            for (var month = 1; month <= 6; month++) {
                ...
                var specificActions = this.getMonthSpecificActions ? this.getMonthSpecificActions(month, currentGrade) : [];
        ...
        try {
            ...
            var report = {
                ...
                creditAssessment: {
                    ...
                    scoreInterpretation: this.interpretCreditScore ? this.interpretCreditScore(this.cibilData.credit_score) : 'Score analysis unavailable',
    ...
    AdvancedAnalytics.prototype.interpretCreditScore = function (score) {
        if (!score || score === 'N/A') return 'No score available';

        var numScore = parseInt(score);
        if (isNaN(numScore)) return 'Invalid score';

        if (numScore >= 800) return 'Excellent - You have an outstanding credit profile';
        if (numScore >= 750) return 'Very Good - You have a strong credit profile';
        if (numScore >= 700) return 'Good - You have a satisfactory credit profile';
        if (numScore >= 650) return 'Fair - Your credit profile needs improvement';
        if (numScore >= 600) return 'Poor - Your credit profile requires significant improvement';
        return 'Very Poor - Your credit profile needs immediate attention';
    };

    // Get target grade based on current grade
    AdvancedAnalytics.prototype.getTargetGrade = function (currentGrade) {
        var gradeOrder = ['F', 'D', 'D+', 'C', 'C+', 'B', 'B+', 'A', 'A+'];
        var currentIndex = gradeOrder.indexOf(currentGrade);

        if (currentIndex === -1) return 'B+'; // Default target

        // Target is one grade higher, max A+
        var targetIndex = Math.min(currentIndex + 1, gradeOrder.length - 1);
        return gradeOrder[targetIndex];
    };

    // Get completion date for improvement plan
    ...
    // Get monthly focus text for improvement plan (used by generateImprovementPlan)
    AdvancedAnalytics.prototype.getMonthlyFocus = function (month, currentGrade) {
        var focusByMonth = {
            1: 'Review all credit accounts for errors and dispute any inaccuracies',
            2: 'Set up automatic payments for at least the minimum amount due',
            3: 'Request a credit limit increase on your credit cards (if you have good payment history)',
            4: 'Consider a secured credit card if you have no active credit accounts',
            5: 'Avoid new credit inquiries unless absolutely necessary',
            6: 'Review your progress and consider professional credit counseling if needed'
        };
        return focusByMonth[month] || 'Continue maintaining good credit habits and monitor your score.';
    };

    // Generate next steps for comprehensive report
    AdvancedAnalytics.prototype.generateNextSteps = function (grade, defaultersCount, utilization) {
        var steps = [];
        if (utilization > 30) {
            steps.push('Reduce credit utilization below 30% for optimal score improvement.');
        }
        if (defaultersCount > 0) {
            steps.push('Contact lenders to resolve any default or overdue accounts.');
        }
        if (grade && grade < 'B') {
            steps.push('Focus on consistent on-time payments to improve your grade.');
        }
        steps.push('Monitor your CIBIL report monthly and dispute any errors.');
        return steps.length ? steps : ['Maintain current credit behavior and monitor your score.'];
    };

    /**
     * Identify priority areas for credit improvement
     */
    AdvancedAnalytics.prototype.identifyPriorityAreas = function () {
        try {
            var priorityAreas = [];
            var scores = this.gradingEngine.getComponentScores ? this.gradingEngine.getComponentScores() : {};

            // Check payment history
            if (scores.paymentHistory < 60) {
                priorityAreas.push({
                    area: 'Payment History',
                    priority: 'HIGH',
                    currentScore: scores.paymentHistory,
                    targetScore: 80,
            ...
            if (scores.creditUtilization < 60) {
                priorityAreas.push({
                    area: 'Credit Utilization',
                    priority: 'HIGH',
                    currentScore: scores.creditUtilization,
                    targetScore: 80,
                    action: 'Reduce credit card balances below 30% of limit'
                });
            }

            // Check credit age
            if (scores.creditAge < 50) {
                priorityAreas.push({
                    area: 'Credit Age',
                    priority: 'MEDIUM',
                    currentScore: scores.creditAge,
                    targetScore: 70,
                    action: 'Keep old accounts open to build credit history'
                });
            }

            // Check recent behavior
            if (scores.recentBehaviour < 60) {
                priorityAreas.push({
                    area: 'Recent Behavior',
                    priority: 'HIGH',
                    currentScore: scores.recentBehaviour,
                    targetScore: 80,
                    action: 'Avoid new credit applications and maintain good payment behavior'
                });
            }

            // If no specific areas, add general improvement
            if (priorityAreas.length === 0) {
                priorityAreas.push({
                    area: 'Overall Credit Profile',
                    priority: 'LOW',
                    currentScore: this.gradingEngine.calculateOverallGrade ? 70 : 0,
                    targetScore: 80,
                    action: 'Maintain current good credit practices'
                });
            }

            return priorityAreas;
        } catch (error) {
            console.error('Error identifying priority areas:', error);
            return [{
                area: 'Credit Profile',
                priority: 'MEDIUM',
                currentScore: 0,
                targetScore: 80,
                action: 'Review credit report and address any issues'
            }];
        }
    };
    ...
    AdvancedAnalytics.prototype.generateRiskAssessment = function () {
        try {
            var riskAssessment = this.riskAssessment;
            if (!riskAssessment) {
                // Create a basic risk assessment if not available
                var accounts = this.creditReport.accounts || [];
                var defaulters = accounts.filter(function (acc) {
                    return acc.paymentStatus === 'DEFAULT' || acc.paymentStatus === 'WRITTEN_OFF';
                });

                var riskLevel = defaulters.length > 0 ? 'HIGH' : 'MEDIUM';
                var creditScore = this.cibilData.credit_score || 0;

                if (creditScore >= 750) riskLevel = 'LOW';
                else if (creditScore >= 700) riskLevel = 'MEDIUM';
                else if (creditScore >= 650) riskLevel = 'MEDIUM-HIGH';
                else riskLevel = 'HIGH';

                return {
                    riskLevel: riskLevel,
                    riskScore: creditScore,
                    defaultProbability: defaulters.length > 0 ? 30 : 10,
                    riskFactors: defaulters.length > 0 ? ['Default accounts detected'] : [],
                    recommendations: defaulters.length > 0 ?
                        ['Clear all defaults immediately', 'Maintain timely payments'] :
                        ['Continue maintaining good credit behavior']
                };
            }
            ...
            return {
                ...
                defaultProbability: riskAssessment.defaultProbability || 10,
            ...
            return {
                ...
                recommendations: ['Review credit report']
            };
        }
    };
     ...
     */
    AdvancedAnalytics.prototype.calculateExpectedImpact = function (current, target) {
        var currentScore = 650;
        if (this.gradingEngine && this.gradingEngine.gradeToScore) {
            currentScore = this.gradingEngine.gradeToScore(current);
        }

        var targetScore = 750;
        if (this.gradingEngine && this.gradingEngine.gradeToScore) {
            targetScore = this.gradingEngine.gradeToScore(target);
        }

        return {
            scoreIncrease: targetScore - currentScore,
            timeframe: '6 months',
            interestSavings: '1-2% on new loans',
    ...
    AdvancedAnalytics.prototype.generateUtilizationTrendData = function () {
        // Mock trend data based on current utilization
        var currentUtil = 30;
        if (this.gradingEngine && this.gradingEngine.getCreditUtilization) {
            currentUtil = this.gradingEngine.getCreditUtilization();
        }

        return {
            labels: ['Month 1', 'Month 2', 'Month 3', 'Month 4', 'Month 5', 'Current'],
            data: [
                Math.min(100, currentUtil + 10),
                Math.min(100, currentUtil + 5),
                Math.min(100, currentUtil + 8),
                Math.min(100, currentUtil + 2),
                Math.min(100, currentUtil + 4),
                currentUtil
            ]
        };
    };
    ...
    AdvancedAnalytics.prototype.generateRiskHeatMapData = function () {
        return {
            paymentHistory: { score: 75, risk: 'Low' },
            utilization: { score: 60, risk: 'Medium' },
            age: { score: 40, risk: 'High' },
            mix: { score: 50, risk: 'Medium' },
            inquiries: { score: 80, risk: 'Low' }
        };
    };
