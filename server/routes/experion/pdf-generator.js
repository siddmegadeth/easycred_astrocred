// PDF Generator for Experion Reports
...
(function () {
    ...
    function ExperionPDFGenerator() {
        this.fontsPath = path.join(__dirname, '../../assets/fonts');
    }

    // Generate Experion PDF Report
    ExperionPDFGenerator.prototype.generateExperionPDF = function (experionData, outputPath, callback) {
        try {
            var doc = new PDFDocument({
                size: 'A4',
                margin: 50,
                info: {
                    Title: 'Experian Credit Report - ' + (experionData.name || 'Report'),
                    Author: 'ASTROCRED',
                    Subject: 'Experian Credit Report Analysis'
                }
            });
            ...
            // Header
            this.addHeader(doc, 'Experian Credit Report', experionData.name);

            // Credit Score Section
            this.addCreditScoreSection(doc, experionData);

            // Profile Information
            this.addProfileSection(doc, experionData);

            // Credit Summary
            this.addCreditSummarySection(doc, experionData);

            // Accounts Section
            this.addAccountsSection(doc, experionData);

            // Analysis Section
            if (experionData.analysis) {
                this.addAnalysisSection(doc, experionData);
            }
    ...
    // Add Header
    ExperionPDFGenerator.prototype.addHeader = function (doc, title, name) {
        ...
        doc.fillColor('#00529b')
            .fontSize(12)
            .text('Powered by Experian', 50, 105, { align: 'center' });
    ...
    ExperionPDFGenerator.prototype.addCreditScoreSection = function (doc, experionData) {
        var y = doc.y;
        var score = parseInt(experionData.credit_score) || parseInt(experionData.score) || 0;

        // Score Box with Experian branding
    ...
    // Add Profile Section
    ExperionPDFGenerator.prototype.addProfileSection = function (doc, experionData) {
        doc.fillColor('#00529b')
            .fontSize(16)
            .font('Helvetica-Bold')
        ...
        var profileData = [
            ['Name:', experionData.name || 'N/A'],
            ['PAN:', experionData.pan || 'N/A'],
            ['Mobile:', experionData.mobile || 'N/A'],
            ['Email:', experionData.email || 'N/A'],
            ['Bureau:', 'Experian India']
        ];
    ...
    ExperionPDFGenerator.prototype.addCreditSummarySection = function (doc, experionData) {
        doc.fillColor('#00529b')
            .fontSize(16)
            .font('Helvetica-Bold')
        ...
        var report = experionData.credit_report && experionData.credit_report[0] ? experionData.credit_report[0] : {};
        var accounts = report.accounts || experionData.accounts || [];
        var enquiries = report.enquiries || experionData.enquiries || [];
    ...
    ExperionPDFGenerator.prototype.addAccountsSection = function (doc, experionData) {
        var report = experionData.credit_report && experionData.credit_report[0] ? experionData.credit_report[0] : {};
        var accounts = report.accounts || experionData.accounts || [];
    ...
    ExperionPDFGenerator.prototype.addAnalysisSection = function (doc, experionData) {
        ...
        var analysis = experionData.analysis || {};
    ...
    // Add Footer
    ExperionPDFGenerator.prototype.addFooter = function (doc) {
        ...
        for (var i = 0; i < pageCount; i++) {
            ...
            doc.fillColor('#999999')
                .fontSize(8)
                .font('Helvetica')
                .text('Generated by ASTROCRED (Experian Data) - ' + new Date().toLocaleDateString(),
    ...
    // Helper Methods
    ExperionPDFGenerator.prototype.getScoreRange = function (score) {
    ...
    ExperionPDFGenerator.prototype.calculateUtilization = function (accounts) {
        var totalLimit = 0;
        var totalBalance = 0;
    ...
    var experionPdfGenerator = new ExperionPDFGenerator();

    // ============== API ENDPOINTS ==============

    // Get Experion PDF Report
    app.get('/get/api/experion/generate-pdf', async function (req, res) {
        try {
            ...
            var fileName = 'Experian_Report_' + (pan || mobile || email) + '_' + Date.now() + '.pdf';
            var outputPath = path.join(outputDir, fileName);

            experionPdfGenerator.generateExperionPDF(experionData, outputPath, function (error, filePath) {
                if (error) {
                    console.error('Error generating Experian PDF:', error);
            ...
            console.error('Error in Experian PDF generation:', error);
            res.status(500).json({
                success: false,
                error: 'Failed to generate PDF',
                details: process.env.NODE_ENV === 'development' ? error.message : undefined
    ...
    module.exports = experionPdfGenerator;
