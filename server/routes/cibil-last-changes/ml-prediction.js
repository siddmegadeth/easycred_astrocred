// File: routes/ml-prediction-api.js
// This should be placed in your routes directory
...
/**
 * Helper function to find CIBIL data by any identifier
 */
async function findCIBILData(mobile, pan, email) {
    try {
        const query = {};
        
        // Build query based on available identifiers
        if (mobile) {
            query.mobile = mobile.replace('+91', '');
        } else if (pan) {
            query.pan = pan.toUpperCase();
        } else if (email) {
        ...
        const cibilData = await CibilDataModel.findOne(query);
        return cibilData ? cibilData.toObject() : null;
        
    } catch (error) {
        console.error('Error finding CIBIL data:', error);
        return null;
    }
}
 ...
 */
function validateParameters(identifiers) {
    const { mobile, pan, email } = identifiers;
    
    if (!mobile && !pan && !email) {
        return { valid: false, error: 'At least one identifier is required (mobile, pan, or email)' };
    }
    
    if (mobile) {
        const cleanedMobile = mobile.replace('+91', '');
...
app.get('/predict', async function(req, res) {
    try {
        ...
        // Validate parameters
        const validation = validateParameters({ mobile, pan, email });
        if (!validation.valid) {
        ...
        // Fetch credit data
        const cibilData = await findCIBILData(mobile, pan, email);
...
app.post('/what-if', async function(req, res) {
    try {
        const { mobile, pan, email, scenarios } = req.body;
...
app.get('/risk-factors', async function(req, res) {
    try {
        // Extract parameters
        const mobile = req.body.mobile || req.query.mobile;
        const pan = req.body.pan || req.query.pan;
...
app.post('/loan-probability', async function(req, res) {
    try {
        const { mobile, pan, email, loanType, amount, tenure } = req.body;
...
function generateAdvancedPredictions(cibilData) {
    ...
    const report = cibilData.credit_report && cibilData.credit_report[0] ? cibilData.credit_report[0] : {};
...
function extractCreditFactors(cibilData) {
    ...
    accounts.forEach(acc => {
        const history = acc.paymentHistory || '';
        if (typeof history === 'string' && history.length > 0) {
            totalPayments += Math.min(history.length, 24); // Consider last 24 months
    ...
    accounts.forEach(acc => {
        const limit = parseFloat(acc.highCreditAmount || acc.highCredit || 0);
    ...
    accounts.forEach(acc => {
        if (acc.dateOpened && acc.dateOpened !== 'NA' && acc.dateOpened !== '11111111') {
            const openDate = new Date(acc.dateOpened);
            if (openDate < oldestAccount) {
                oldestAccount = openDate;
                hasValidDate = true;
            }
        }
    });
    const creditAgeYears = hasValidDate ? Math.max(0, (new Date() - oldestAccount) / (365.25 * 24 * 60 * 60 * 1000)) : 0;

    // Credit mix
    const accountTypes = new Set();
    accounts.forEach(acc => {
        if (acc.accountType) {
            accountTypes.add(acc.accountType);
        }
    });
    ...
    const recentInquiries = inquiries.filter(inq => {
        if (!inq.enquiryDate) return false;
        const date = new Date(inq.enquiryDate);
...
function analyzeBehavioralPatterns(cibilData) {
    ...
    // Payment consistency
    let consistentPayments = true;
    let missedPaymentCount = 0;
    
    accounts.forEach(acc => {
        const history = acc.paymentHistory || '';
        if (history.includes('1') || history.includes('2') || history.includes('3')) {
    ...
    const behavioralScore = consistentPayments ? 85 : 65;

    return {
        behavioralScore: behavioralScore,
        patterns: {
            spendingTrend: totalSpending > 500000 ? 'High' : totalSpending > 100000 ? 'Moderate' : 'Low',
            paymentConsistency: consistentPayments ? 'Consistent' : 'Irregular',
            creditDependency: accounts.length > 5 ? 'High' : accounts.length > 2 ? 'Moderate' : 'Low',
            missedPayments: missedPaymentCount
        },
        insights: [
            consistentPayments ? 'Your payment patterns are stable and consistent' : 'You have some irregular payment patterns',
            'Consider maintaining a healthy credit mix for optimal scoring'
        ]
    };
}
